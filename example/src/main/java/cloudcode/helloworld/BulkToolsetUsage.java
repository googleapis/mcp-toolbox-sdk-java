...
/**
 * Example demonstrating how to use the advanced Bulk Toolset Loading API.
 * This showcases how to pre-bind parameters and credentials to multiple tools
 * at startup, retrieving a map of fully configured and ready-to-use Tool objects.
 */
public class BulkToolsetUsage {
    public static void main(String[] args) {
        ...
        try {
            ...
            if (keyPath != null && !keyPath.isEmpty() && !keyPath.contains("YOUR_CREDENTIALS_JSON_FILE_PATH")) {
            ...
            toyParams.put("description", "teddy bear"); // Pre-bind "description" to "teddy bear" for "get-toy-price"
            ...
            AuthTokenGetter toolAuthGetter = () -> CompletableFuture.completedFuture("dummy-auth-token-from-provider");
            toyAuth.put("google_auth", toolAuthGetter); // Pre-bind "google_auth" service for "get-toy-price"
            ...
            client.loadToolset(null, paramBinds, authBinds, true)
                .thenAccept(boundTools -> {
                    System.out.println("\n[1] Toolset loaded in bulk. Total tools bound: " + boundTools.size());

                    // Let's verify and execute "get-retail-facet-filters" (simple tool, no pre-bindings)
                    Tool filterTool = boundTools.get("get-retail-facet-filters");
                    if (filterTool != null) {
                        System.out.println("\n[2] Executing 'get-retail-facet-filters'...");
                        filterTool.execute(Map.of())
                            .thenAccept(result -> {
                    ...
                    if (priceTool != null) {
                        System.out.println("\n[3] Executing 'get-toy-price' (using pre-bound parameter 'teddy bear')...");
                        // We run execute with empty map, so it falls back to the bound 'teddy bear'
                        priceTool.execute(Map.of())
                            .thenAccept(result -> {
                                System.out.println("    -> Result (Teddy Bear): " + result.content().get(0).text());
                            }).join();

                        System.out.println("\n[4] Attempting to override parameter at runtime to 'barbie' (should be ignored/overridden by bound value)...");
                        // We pass description at runtime but it will be overridden by bound "teddy bear"
                        priceTool.execute(Map.of("description", "barbie"))
                            .thenAccept(result -> {
                                System.out.println("    -> Result (Actual): " + result.content().get(0).text() + " (Expect price for teddy bear: 14.99)");
                            }).join();
                    } else {
